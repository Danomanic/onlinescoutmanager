...
describe("members.list", () => {
  it("fetches member list for a section and term", async () => {
    const data = {
      identifier: "scoutid",
      photos: true,
      items: [{ scoutid: 1, firstname: "Jamie" }],
    };
    const fetchMock = mockFetch(
      new Map([["getListOfMembers", { body: data }]]),
    );
...
describe("members.get", () => {
  it("fetches individual member details", async () => {
    const data = {
      ok: true,
      read_only: [],
      data: { scoutid: "42", firstname: "Jamie" },
      meta: [],
    };
...
describe("members.getPatrols", () => {
  it("fetches patrols with members", async () => {
    ...
    const fetchMock = mockFetch(
      new Map([["getPatrolsWithPeople", { body: data }]]),
    );
